class BoletosApartados extends HTMLElement{
    ...
    connectedCallback() {
        const administrador = this.getAttribute("administrador");
        const shadow = this.attachShadow({ mode: "open" });
        this.#render(shadow);
        this.#agregarEstilos(shadow);
        this.#cargarBoletosApartados(shadow, administrador);
    }
    ...
    #cargarBoletosApartados(shadow){
        let cuerpoTabla = shadow.querySelector("#cuerpo-tabla");

        fetch(this.#urlBoletos + "?idCliente=" + idCliente).then((response) => {
            ...
            <tr>
                <td>
                    <div class="d-flex px-2 py-1">
                        ...
                        <h6 class="mb-0 text-sm">Número 20</h6>
                        <p class="text-xs text-secondary mb-0">Laura López</p>
                        </div>
                    </div>
                </td>
                <td>
                    <p class="text-xs font-weight-bold mb-0">Comprobante20.jpg</p>
...
window.customElements.define('boletos-pagados', BoletosApartados)